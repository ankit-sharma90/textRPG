...
class World:
    ...
    def get_viewport_map(self, viewport_size=5):
        """Get a 5x5 viewport around the player showing discovered areas"""
        current_map = self.get_current_map()
        viewport = []
        
        # Calculate viewport bounds
        half_size = viewport_size // 2
        start_x = max(0, self.player_x - half_size)
        end_x = min(50, self.player_x + half_size + 1)
        start_y = max(0, self.player_y - half_size)
        end_y = min(50, self.player_y + half_size + 1)
        ...
        for y in range(start_y, end_y):
            ...
            for x in range(start_x, end_x):
                ...
                elif current_map.has_major_event_at(x, y):
                    row.append("!")
                else:
                    row.append("·")
            viewport.append(row)
        ...
        return {
            ...
            'world_name': self.current_world.value,
            'viewport_bounds': (start_x, start_y, end_x, end_y)
        }
