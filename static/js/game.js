...
const playerBattleHealthBar = document.getElementById(
  "player-battle-health-bar"
);
const playerBattleHealthText = document.getElementById(
...
function executePlayerTurn(data, choice) {
  ...
  setTimeout(() => {
    ...
    // 0.75s delay (0.25s + 0.5s extra), then play attack animation
    setTimeout(() => {
      let animationDuration = 250; // Default fallback
      if (choice <= gameState.options.length) {
        animationDuration = triggerSkillAnimation(
    ...
    }, 750); // Changed from 250 to 750 (added 500ms)
  }, 250);
}
...
function executeEnemyTurn(data) {
  ...
  let enemyActionLine = lines.find((line) => {
    const lower = line.toLowerCase();
    return (
      (lower.includes("goblin") || lower.includes("enemy")) &&
      (lower.includes("attack") ||
        lower.includes("hit") ||
        lower.includes("strike") ||
        lower.includes("damage")) &&
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      if (enemyActionLine) {
        // Check if this is a "no damage" message and replace with attack message
        const lowerLine = enemyActionLine.toLowerCase();
        if (
          lowerLine.includes("no damage") ||
          lowerLine.includes("did no damage")
        ) {
          // Show generic attack message instead of "no damage" message
          const enemyName = gameState.enemy ? gameState.enemy.name : "Enemy";
          addBattleLogMessage(`${enemyName} attacks!`, false);
        } else {
          addBattleLogMessage(enemyActionLine, false);
        }
      }

      // 0.75s delay (0.25s + 0.5s extra), then play enemy attack animation
      setTimeout(() => {
        const animationDuration = triggerSkillAnimation(
          "attack",
          false,
          enemyActionLine
        );
        ...
        setTimeout(() => {
          // Calculate actual damage taken by comparing health before and after
          const currentHealth = parseInt(
            document.getElementById("health").textContent.split("/")[0]
          );
          const newHealth = data.player.health;
          const actualDamage = currentHealth - newHealth;

          // Extract expected damage from message for comparison
          const expectedDamage = extractDamageFromMessage(enemyActionLine);

          // Show the correct damage number based on actual damage taken
          const playerBattleInfo =
            document.getElementById("player-battle-info");
          if (playerBattleInfo && actualDamage >= 0) {
            if (actualDamage === 0) {
              // If no actual damage was taken, show "BLOCKED!" and update battle log
              showBlockedMessage(playerBattleInfo);
              // Show "The attack was blocked!" message when BLOCKED animation appears
              setTimeout(() => {
                addBattleLogMessage("The attack was blocked!", false);
              }, 100); // Small delay to sync with BLOCKED animation
            } else {
              // Show actual damage taken
              showDamageNumber(playerBattleInfo, actualDamage, false);
            }
          }
        ...
        // Wait for all animations to complete before continuing
        setTimeout(() => {
          // If we showed a "blocked" message, add extra delay to let it be visible
          const battleLogMessage = battleLog.textContent;
          if (battleLogMessage && battleLogMessage.includes("blocked")) {
            // Add extra delay for blocked message to be visible
            setTimeout(() => {
              battleAnimating = false;
              setBattleActionsEnabled(true);
            }, 1500); // 1.5 second delay for blocked message
          } else {
            battleAnimating = false;
            setBattleActionsEnabled(true);
          }
...
function processMessage(message, event) {
  if (event === "battle") {
    ...
    document.querySelector(".current-message-container").style.display =
      "block";
    clearCurrentMessage();
    ...
    if (playerDefeated) {
      showDefeatedScreen(
        `You were killed by ${killedBy || "your enemy"}`,
        killedBy
      );
    } else if (nonBattleLines.length > 0 && defeatedMonster) {
      showSuccessScreen(
        `Victory!`,
        `You defeated ${defeatedMonster}`,
        goldGained
      );
    } else {
...
function hideBattleInfo() {
  ...
  // Show status bar again when battle ends
  if (statusBar) {
...
function triggerSkillAnimation(
  ...
  if (
    ...
    if (isPlayerAction) {
      ...
      maxHealth =
        document.getElementById("health").textContent.split("/")[1] || 10;
    ...
    setTimeout(() => {
      if (damage > 0) {
        // For enemy attacks, we'll handle damage numbers in executeEnemyTurn to show actual damage
        if (isPlayerAction) {
    ...
    lowerAction.includes("defend") ||
    lowerAction.includes("block") ||
    lowerAction.includes("guard")
  ) {
  ...
  return Math.max(
    parseFloat(currentSpeed.damageTime) * 1000,
    parseFloat(currentSpeed.iconTime) * 1000
  );
}
...
// Show blocked message floating up from health bar
function showBlockedMessage(targetElement) {
  if (!targetElement) return;

  const blockedElement = document.createElement("div");
  blockedElement.classList.add("damage-number");
  blockedElement.textContent = "BLOCKED!";
  blockedElement.style.color = "#6666ff"; // Blue color for blocked

  // Apply current animation speed directly to the element
  const currentSpeed = ANIMATION_SPEEDS[currentAnimationSpeed];
  blockedElement.style.animation = `damage-float ${currentSpeed.damageTime} ease-out forwards`;

  targetElement.appendChild(blockedElement);

  // Remove the element after animation completes (use longer timeout for slower animations)
  const timeoutDuration = parseFloat(currentSpeed.damageTime) * 1000;
  setTimeout(() => {
    if (blockedElement.parentNode) {
...
function showActionIcon(targetElement, actionType, damage = 0, maxHealth = 0) {
  ...
  if (damage > 0 && maxHealth > 0 && actionType === "attack") {
    ...
    // Debug logging
    console.log(
      `Icon sizing: damage=${damage}, maxHealth=${maxHealth}, percentage=${damagePercentage.toFixed(
        1
      )}%`
    );
...
function extractDamageFromMessage(message) {
  ...
  const damageMatch =
    message.match(/(?:deals?|takes?|suffers?)\s+(\d+)\s+damage/i) ||
...
function applyAnimationSpeed() {
  ...
  console.log(
    "Applied animation speed:",
    currentSpeed.name,
    "Damage:",
    currentSpeed.damageTime,
    "Icons:",
    currentSpeed.iconTime
  );
}
...
function setBattleActionsEnabled(enabled) {
  ...
  if (battleActions && battleLog) {
    if (enabled) {
      ...
      console.log(
        "Battle actions has centered class:",
        battleActions.classList.contains("centered")
      );
      console.log(
        "Battle actions computed style:",
        window.getComputedStyle(battleActions).display
      );
      console.log(
        "Battle actions children count:",
        battleActions.children.length
      );
    } else {
    ...
    console.log(
      "Missing elements - battleActions:",
      !!battleActions,
      "battleLog:",
      !!battleLog
    );
...
function takeAction(choice) {
  ...
  if (gameState.event === "battle" && gameState.enemy) {
    ...
    // Clear battle log immediately to prevent old message from showing
    while (battleLog.firstChild) {
      battleLog.removeChild(battleLog.firstChild);
    }
